...
def split_stuck_words(word: str) -> str:
    # contoh hardcode common patterns
    patterns = [
...
def get_main_keyword(keywords, stop_words_id, top_n=2):
    ...
    # Hilangkan duplikat tapi jaga urutan
    tokens = list(dict.fromkeys(tokens))

    if not tokens:
        return keywords[0].lower()

    # Ambil hanya top_n kata unik
    tokens = tokens[:top_n]
...
def clean_main_keyword(keyword: str) -> str:
    ...
    # Hapus mentions & cs-username
    keyword = re.sub(r'@\w+', '', keyword)
    keyword = re.sub(r'\b(cs[_-]*\w+)\b', '', keyword)

    # Ubah non-huruf jadi spasi
    keyword = re.sub(r'[^a-z]', ' ', keyword)

    # Rapikan spasi → pecah kata → join dengan underscore
...
# === Streamlit UI (tanpa pengaturan manual) ===
